...
//===========================================================================================================================================
// The functions b/w these headers are the precursor to the above Bezier by degree, not sure they have any use rn but Leaving for ref sake
//===========================================================================================================================================
inline real32 BezierByDegree(real32 point[], real32 time, int deg) { //Really proud of this even if it's just a foundation
    //Better this later so you can continue to utilize it in other projects.
    //@TODO remove clamp of 8
    //@INFO this is a really shitty way of doing this dynamic Arrays would be better
    if (deg > 8) {
        Print("Degree cannot be greater than 8");
        ASSERT(deg < 8);
    }
    else {
        real32 t[9];
        ...
        for (int i = 0; i <= deg; i++) {
            ...
            pieceOut = mt[deg - i] * point[i] * t[i] * tempDeg;
            outValue += pieceOut;
        }
        return outValue;
    }
...
inline vec2 BezierByDegree(vec2 point[], real32 time, int deg) {
    real32 xPoints[8];
    real32 yPoints[8]; //@TODO convert these from arrays to vectors so that the degree is not capped.
                        //@TODO have the degree be direclty tied to the number of points fed. instead of an input
    ...
    if (deg > 8) {
        ...
        return V2(BezierByDegree(xPoints, time, deg), BezierByDegree(yPoints, time, deg));
    }
}
//===========================================================================================================================================
// The functions b/w these headers are the precursor to the above Bezier by degree, not sure they have any use rn but Leaving for ref sake
