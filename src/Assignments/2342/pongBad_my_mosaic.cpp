...
//=========================================================================
    ...
    void PlayBGMusic() {
        if (gameOver == false && handle.generation == 0) {
            handle = PlaySound(&Game->audioPlayer, bgMusic, .5f, true);
        }

    }

    void PlayHitSound() {
        /*if (hitHandle.generation == 0) {
            hitHandle = PlaySound(&Game->audioPlayer, hitSound, 1.0f, false);
        }*/
        PlaySound(&Game->audioPlayer, hitSound, 1.0f, false);
    }
...
void EndGame() {
    ...
    if (playerScore->count >= 11) {
        ...
        LoadSoundClip("data/sfx/Pong_Success_01.wav", &endSound);
        PlayEndSound();
    ...
    if (compScore->count >= 11) {
        ...
        LoadSoundClip("data/sfx/Pong_Failure_01.wav", &endSound);
        PlayEndSound();
    }
}
...
void BallBoundaries() {  //@TODO need to calculate nPos here as well so the ball doesn't glitch into the paddle on certain corners and hitboxes
    ...
    if ((ball->velocity.x > 0) && (ball->pos.x > ball->max.x)) { //@Q am i switching x dir here? i'm very confused why am i switching x dir on wall impact, and why does the game not switch???
        ...
        LoadSoundClip("data/sfx/Player_Score_01.wav", &scoreSound);
        PlayScoreSound();
        ...
        LoadSoundClip("data/sfx/AI_Score_01.wav", &scoreSound);
        PlayScoreSound();
    }
...
void MyMosaicInit() {
    ...
    LoadSoundClip("data/sfx/BassLine_Game44100.wav", &bgMusic);
    LoadSoundClip("data/sfx/paddleHit_01.wav", &hitSound);
    Sound* sound = GetSound(&Game->audioPlayer, handle);
    sound->volume = 1.0f;
