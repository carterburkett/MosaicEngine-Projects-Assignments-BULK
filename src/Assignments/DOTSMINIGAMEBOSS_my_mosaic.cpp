...
void GameOver() {
    ...
    if (!player->alive) {
        ...
        DrawTextScreenPixel(&Game->serifFont, V2(800, 400), 20, WHITE, true, "Hit 'R' To Play Again or 'ESC' to Quit");
    }
}
...
void MyMosaicUpdate() {
    ...
    if (hwLevel->player.alive) {
        ...
        if (hwLevel->bossFight) {
            ...
            if (hwLevel->opp.count < 4) {
                ...
                Print("Enemy count = %i", hwLevel->opp.count);
            }

            DrawEnemies();

            if (hwLevel->score >= 16 && !hwLevel->bossFight) {//CHANGE THIS BACK TO 15
    ...
    if (InputPressed(Keyboard, Input_R)) { hwLevel->restart = true; }
